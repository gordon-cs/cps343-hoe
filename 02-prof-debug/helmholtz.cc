 ...
 * iteration.
 *
 * $Smake: g++ -Wall -O2 -o %F %f -lm
 *
 * To profile add "-pg" to compiler command line and use "-O" instead of "-O2":
 *     g++ -pg -Wall -O -o %F %f -lm
 *
 * To change number of iterations between convergence checks define ITER_STEP
 * to be the desired value.  For example:
 *     g++ -DITER_STEP=100 -pg -Wall -O2 -o %F %f -lm
 *
...
// Define constants that set problem size.  NX and NY are the number of
// subintervals in the x and y directions.  They are used below to
// initialize nx = NX + 1 and ny = NY + 1, which specify the number of
// grid points in the x and y directions.  By default the domain is square
// and both NX and NY are set the value of N.  At compile time a square
// domain can be configured by defining the value of N with "-DN=<value>".
// To configure a rectangular domain, use "-DNX=<value>" and "-DNY=<value".

#if !defined(N)   // default value for both NX and NY
#define N 799
#endif

#if !defined(NX)  // subintervals in x direction, results in NX+1 grid points
#define NX N
#endif

#if !defined(NY)  // subintervals in y direction, results in NY+1 grid points
#define NY N
#endif

#if !defined(ITER_STEP)  // iterations between convergence checks
#define ITER_STEP 1
#endif
...
void initializeDomain( double** u, double h, int nx, int ny )
{
    ...
    // interpolate (linear) boundary data across domain
...
{
    int nx = NX + 1;       // number of grid points in x direction
    int ny = NY + 1;       // number of grid points in y direction

    double h = 1.0 / NX;   // use horizontal spacing as grid spacing

    // Create the grid.  We must have a constant stride in each
    // dimension.  This is accomplished by allocating an array of
    // pointers then allocating the full data array to the first
    // pointer.  The remaining pointers are set to point to the start
    // of each "row" of contiguous data in the single linear array.
    ...
    const double w = 2.0
        / ( 1.0 + 0.5 * ( sin( M_PI / NX ) + sin( M_PI / NY ) ) );
