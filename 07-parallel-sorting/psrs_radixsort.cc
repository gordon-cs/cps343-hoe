 ...
 * Revised March 2022 - Created radix sort version
 * Revised March 2024 - Added command-line switch for timing information
 *
 * Implements Parallel Sorting with Regular Sampling (PSRS) using a
 * sequential Radix Sort for local sorting.  Radix sort implementation
 * derived from presentation at http://www.cubic.org/docs/radix.htm.
 */
...
//-----------------------------------------------------------------------------
//
// Counting sort.  Assumes list is an integer datatype and sorts on the byte
// value indicated by the input variable offset.
//
// Input:
//    unsigned int offset  - offset of byte to sort on, counted from the least
//                             significant byte.  When the list is of type int
//                             on a machine with 4-byte integers, then the
//                             offset should be one of 0, 1, 2, or 3.
//    unsigned int n       - the length of the list to be sorted
//    int* source          - pointer to list to be sorted
//
// Output:
//    int* dest            - pointer to sorted list
//
// Derived from code found at http://www.cubic.org/docs/radix.htm.

void countingsort(size_t offset, size_t n, int *source, int *dest)
{
    // to change the data type of the list to something other than int,
    // the type of pointers in the function declaration must be changed
    // and the argument of the sizeof() function and the type of the
    // *sp pointer must be changed in the next two lines.
    const unsigned int datasize = sizeof(int);
    int* sp;                    // pointer into source array

    unsigned int count[256];    // holds freq. of bytes then location of bytes
    unsigned int* cp;           // pointer into count array
    unsigned char* bp;          // byte pointer
    unsigned int i;             // for-loop index
    unsigned int sum, tmp;      // temporary variables

    // zero out count[] array
    for (i = 0, cp = count; i < 256; i++) *cp++ = 0;
...
//-----------------------------------------------------------------------------
//
// Radix sort.  Assumes list is a 4-byte integer datatype and performs a
// radix sort on the these byte value using a stable counting sort
//
// Input:
//    int* source          - pointer to list to be sorted
//    unsigned int n       - the length of the list to be sorted
//
// Output:
//    int* source          - pointer to sorted list (sorted in place)
//
// Derived from code found at http://www.cubic.org/docs/radix.htm.

void radixsort(int *source, unsigned int n)
{
    int* temp = new int [n];
    countingsort(0, n, source, temp);
    countingsort(1, n, temp, source);
    countingsort(2, n, source, temp);
    countingsort(3, n, temp, source);
    delete [] temp;
}
